...
:: === COPY FILE ===
set "botFile=setup\lumir-mt5.ex5"
echo "%botFile%"
...
echo.
set /p choice="Chon so instance de sua TradeHistoryPeriod: "
...
for /f "usebackq delims=" %%A in ("%configFile%") do (
  set "line=%%A"
  if /I "!line:~0,18!"=="TradeHistoryPeriod" (
    echo TradeHistoryPeriod=7
    set "found=1"
  ) else (
    echo !line!
  )
...
if "!found!"=="0" (
  ...
  echo TradeHistoryPeriod=7>>"%tempFile%"
)

echo Da sua thanh cong TradeHistoryPeriod=7

move /y "%tempFile%" "%configFile%" >nul

echo === FILE BOT ===
set "targetExperts=!selected!\MQL5\Experts"
